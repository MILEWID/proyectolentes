...
//funcion modificar
    function modificar( $id){
        ...
        // intanciamos la conexion
        $conexion = new Cconexion();
        $query= "UPDATE FROM usuario SET (nombreUsuario, nombre, apellido, tipoUsuario, correoElectronico, contrasena) VALUES('$nomusu', '$nombre', '$apellido','$tipusu', '$email', '$contra') WHERE idUsuario='$id'";
         $resultado = $conexion->conectar()->query($query);
        if ($resultado){
            return "ok";
        }else{
            echo "No existe el registro que desea editar";
        }
